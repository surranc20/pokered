...
class LevelManager(object):
    def __init__(self, player, level_name, screen_size =(240, 160), movie=None):
        ...
        NOTE: Right now these events are called self._active_event. I will change this name later but
    ...
    def handle_event(self, event):
        ...
        handle the event."""
        if self._active_event == None:
            self._active_event = self._player.handle_event(event, self._level.get_nearby_tiles(self._player._current_tile._pos))
        else:
    ...
    def update(self, ticks):
        ...
        # Update the current event.
            ...
            if self._active_event.is_over():
                if self._active_event.get_end_event() != None:
