...
class TextCursor(Drawable):
    ...
    def reset(self):
        self._position = self._anchored_pos
    ...
    def update(self, ticks):
        ...
        if self.should_update:
            if not self._invert:
                ...
                elif self._position[main_axis] - self._anchored_pos[main_axis] >= 2:  # NOQA
                    self._current_delta = - 1
            else:
                if self._position == self._anchored_pos:
                    self._current_delta = - 1
                elif self._anchored_pos[main_axis] - self._position[main_axis] >= 2:  # NOQA
                    self._current_delta = 1
