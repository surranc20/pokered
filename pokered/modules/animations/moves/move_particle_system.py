...
class MoveLinearParticleSystem():
    def __init__(self, file_name, index_pos, start_pos, total_x_displacement, dx=1, dy=-1, particles_per_second=10, duration = 100):
        """Creates a particle system. Used in moves like ice beam. Takes as its input a file_name which specefies which file
        to look in for the individual particles. The index_pos which specifies the x offset to use in order to find the correct
        particle sprite. The start pos which specifies the start position of the particles The total x displacement that a particle
        will travel before dying. The dx and dy which specefies how much the particles position changes per update. Particles per
        second which determines how many particles spawn per second, and the duration in seconds for how long the system will last."""
