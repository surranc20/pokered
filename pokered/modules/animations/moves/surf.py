...
class Surf(MoveBase):
    ...
    def __init__(self, attacker, defender, enemy=False):
        """Creates the surf animation. The move extends MoveBase but overwrites most of the logic."""
        ...
        # The location of the transparent pixel is different based on which surf image is used
        if self._enemy:
    ...
    def update(self, ticks):
        """Updates the surf animation. Essentially it moves the surf png diagonally anf fill in the levels the
        png has already travelled with blue in order to give the appearance of a wave. """
        ...
        if self._animation_timer > 1 / self._fps:
            ...
            # If the user uses the move then the wave travels upwards diagonally and fills the area
            # beneath the wave with blue.
            if not self._enemy:
            ...
            # If the enemy used the move then the wave travels diagonally downward and fills the area
            # above the wave with blue.
            else:
