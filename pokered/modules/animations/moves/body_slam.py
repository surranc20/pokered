...
class BodySlam(MoveBase):
    ...
    def __init__(self, attacker, defender, enemy=False):
        """This is the animation for the move body slam. It first performs a
        vertical drop and recover, then a horizontal lunge, then displays the
        impact png, the the enemy performs a horizontal lunge, then the enemy
        jiggles, then it lunges back to its start point and the animation ends.
        Extends MoveBase"""
        ...
        self._reverse_lunge = HorizontalLunge(attacker,
                                              -10 if not enemy else 10)
        self._enemy_lunge = HorizontalLunge(defender, 14 if not enemy else -14)
        self._enemy_recover = HorizontalLunge(defender,
                                              -14 if not enemy else 14)
        ...
        # This is one of the few animations where Move Base's attempt to
        # adjust the frame_list's frame's positions does not work well so we
        # will do this manually. This makes sure that when the enemy
        # uses body slam the impact png will display to the right of the
        # player's pokemon.
        if enemy:
            self.FRAME_LIST = [
                [(0, (175, 20))],
    ...
    def update(self, ticks):
        ...
        # Once the impact sprite has been displayed the enemy is knocked
        # (lunges)
        ...
        # Once the enemy is done jiggling the attacker lunges back to its
        # start position
        elif not self._reverse_lunge.is_dead():
            self._reverse_lunge.update(ticks)

        # Once the attacker has returned to his start position the enemy
        ...
        # This resets the animations is_dead variable which gets flipped to
        # true by super()
        ...
        # This creates the osciallation animation. It must be made here
        # because it needs to be created
        # after the defending pokemon has been knocked into the position it
        # will jiggle in.
