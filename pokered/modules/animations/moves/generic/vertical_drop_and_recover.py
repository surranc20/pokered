...
class VerticalDropAndRecovery():
    def __init__(self, pokemon, drop_dist):
        """The pokemons dip down by the distance specefied in drop_dist and then quickly rebounds up."""
        self._pokemon = pokemon
        self._fps = 5

        # Start the timer with just enough initial value that the first frame updates immediately
    ...
    def draw(self, draw_surface):
        """Since this animation only changes the position of the pokemon it does not need to do 
        anything in the draw method"""
        pass
    ...
    def reset(self):
        """Safety feature. Ensures the pokemon is back in its original position and then ends animation."""
    ...
    def update(self, ticks):
        ...
        if not self.is_dead():
            ...
            if self._timer > 1 / self._fps:
                ...
                # If the pokemon has not moved yet move down by the drop distance
