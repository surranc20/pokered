...
class Movie():
    def __init__(self, folder_name):
        """Creates a movie instance. I could not find a movie player that
        works well with pygame and won't close my game when playing the movie,
        so I decided to make my own. It takes the name of the folder where
        ...
        self._frame = pygame.image.load(join(self._folder_name,
                                             "scene00001.png"))
        self._frame.convert()
    ...
    def update(self, ticks):
        """Update the frame of the movie. Ensures that the movie plays at the
        correct fps."""
        self._timer += ticks
        if self._timer > 1 / self._fps:
            ...
            try:
                self._frame = pygame.image.load(join(self._folder_name,
                                                     ...
                                                     + ".png"))
                self._frame.convert()
            # This will trigger once the movie has played all the way through.
            # Right now the movie just restarts
            except Exception:
                self._counter = 1
                self._frame = pygame.image.load(join(self._folder_name,
                                                     "scene" + self._get_num()
    ...
    def handle_event(self, event):
        """Handles the events for the movie player. Ends the video if the
        event is the select action."""
    ...
    def _get_num(self):
        """Ensures that the counter object has the necessary padding of
        zeros."""
