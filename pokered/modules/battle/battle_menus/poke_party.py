...
class PokeParty(Drawable):
    def __init__(self, player):
        """Creates a poke party. This is what appears when you are choosing a
        pokemon. Shows the pokemon in the players current party."""
    ...
    def _blit_secondary_pokemon(self):
        """Creates the other display objects. These are the pokemon that are
        not in position one."""
        position = (88, 9)
    ...
    def draw(self, draw_surface):
        """Draws the active pokemon, all secondary pokemon, the cancel button,
        and the text box"""
        ...
        if self._pokemon_selected_menu is not None:
            self._pokemon_selected_menu.draw(draw_surface)

    def change_cursor_pos(self, action):
        """Changes the cursor position. The cursor keeps track of which
        pokemon is selected. or where you are in the pokemon selected menu if
        a pokemon is selected."""
        if self._selected_pokemon is False:
            ...
            if old_pos != self._cursor:
                self._update_selected_pos(old_pos)
        else:
    ...
    def handle_back_event(self):
        """Handles a back event. If the the active pokemon is dead then a
        small error sound is played. If a poke menu is open, then the window
        is closed. Otherwise, the poke party closes."""
    ...
    def handle_select_event(self, action):
        """Handles a select event. If the user selects cancel then they return
        to the battle screen. Otherwise, a variety of checks are performed to
        ...
        if self._cursor == 6:
            ...
            if not self._selected_pokemon:
                ...
                self._pokemon_selected_menu = PokemonSelectedMenu(self._player,
                                                                  self._cursor,
                                                                  battle=True)
                self._text_bar.blit_string("Do what with this PKMN?")

            # Determine whether or not the pokemon can be sent into battle and
            # act accordingly.
            else:
    ...
    def update(self, ticks):
        """This update method allows the pokemon sprites to bounce up and
        down."""
        for item in self._selectable_items:
            item.update(ticks)
...
class PokemonMenuPokemon(Drawable):
        ...
        if not self._pokemon.is_alive():
            if selected:
                ...
                pygame.transform.threshold(
                    ...
                    self._image.copy(),
                    (56, 144, 216),
                    set_color=(192, 104, 16),
                    inverse_set=True)
                pygame.transform.threshold(
                    self._image,
                    self._image.copy(),
                    (128, 192, 216),
                    set_color=(208, 160, 32),
                    inverse_set=True)
                pygame.transform.threshold(
                    self._image,
                    self._image.copy(),
    ...
    def _blit_hp_remaining(self):
        ...
        # This ensures that hp displays correctly even if it is less than
        # three digits.
        ...
        for char in str(str(current_hp) + " " + str(self._pokemon.stats["HP"])):
            font_index = int(ord(char)) - 48
            font_char = FRAMES.getFrame(
                "party_font.png", offset=(font_index, 1))
            font_char.set_colorkey((0, 128, 0))
            if char == " ":
                pass
            else:
                self._hp_remaining.blit(
                    font_char, (current_pos.x, current_pos.y))
            current_pos.x += 5
...
class ActivePokemon(PokemonMenuPokemon):
    ...
    def draw(self, draw_surface):
        ...
        draw_surface.blit(
            self._lvl, (self._position[0] + 46, self._position[1] + 30))
        draw_surface.blit(self._hp_remaining,
                          (self._position[0] + 44, self._position[1] + 46))
        draw_surface.blit(
            self._hp, (self._position[0] + 30, self._position[1] + 41))
        draw_surface.blit(
            self._hp_darken, (self._position[0] + 30, self._position[1] + 41))
...
class SecondaryPokemon(PokemonMenuPokemon):
    ...
    def draw(self, draw_surface):
        ...
        self._bouncing_pokemon.draw(draw_surface)
        draw_surface.blit(
            self._hp, (self._position[0] + 96, self._position[1] + 9))
        draw_surface.blit(
            self._hp_darken, (self._position[0] + 96, self._position[1] + 9))
        draw_surface.blit(self._hp_remaining,
                          (self._position[0] + 110, self._position[1] + 15))
        draw_surface.blit(
            self._lvl, (self._position[0] + 48, self._position[1] + 15))
...
class BouncingPokemon(Animated):
    def __init__(self, pokemon, position):
        ...
        _offset = (0, _lookup)
        super().__init__(join("pokemon", "pokemon_small.png"), position,
                         offset=_offset)
        self._nFrames = 2
        self._row = _lookup
...
class CancelButton(Drawable):
    def __init__(self, selected=False):
        """Creates a simple cancel button which is seen in the bottom right of
        the screen. Has a different appearance based on whether or not it is
        selected."""
...
class PartyTextBar(Drawable):
    def __init__(self):
        """"Creates the text bar which displays in the bottom right of the
        pokemon party menu screen."""
    ...
    def blit_string(self, string):
        ...
        for char in string:
            ...
            # The lower case i char is much smaller than other characters and
