...
class PokeInfo(Drawable):
    ...
    def __init__(self, pokemon, enemy=False, hp=None):
        ...
        the pokemon's name, hp, level, status, as well as exp status if it is
        the players pokemon. Arguments are a Pokemon and boolean that
        specifies whether or not this is an info bar of an enemies pokemon"""
        self._enemy = enemy
        self._pokemon = pokemon
        self._current_hp = hp

        if not enemy:
            super().__init__(join("battle", "health_bars.png"),
                             self.PLAYER_POKE_INFO_POS, offset=(0, 1))
        else:
            super().__init__(join("battle", "health_bars.png"),
                             self.ENEMY_POKE_INFO_POS, offset=(0, 0))
    ...
    def _blit_name_and_gender(self):
        ...
        self._text_image = pygame.Surface((self._image.get_width(),
        ...
        for char in self._pokemon.nick_name.lower():
            ...
            font_char = FRAMES.getFrame("pokemon_fire_red_battle_font.png",
                                        offset=(font_index, 0))
    ...
    def _blit_hp_remaining(self):
        ...
        # Blit hp remaining to hp remainging surface.
        for char in str(str(current_hp) + "/" +
    ...
    def is_dead(self):
        """Returns whether or not the instance is dead. An instance is dead if
        its pokemon is dead."""
        return not self._pokemon.is_alive()

    def draw(self, draw_surface):
        """Draws the various subsurfaces to the screen in the correct location
        based on whether or not the pokemon is an enemy."""
        super().draw(draw_surface)
        draw_surface.blit(self._text_image,
    ...
    def update(self, ticks):
        """Updates the poke info. In reality it just creates another poke info
        to ensure the current hp is always displayed correctly. This can be
        made more effecient but as of right now it is not a problem."""
